 ...
 *
 */
function getTinyUrl($url, $timeout = 3600 ) {

	// If cache exists send it back
	if ( $out = get_transient( 'pagelines_tinyurl_' . md5( $url ) != false ) ) return $out;

	// Fetch the short url from tiny api
	...
	// Check the body from tinyurl is actually a url and not a 400 error
	// If its OK we will cache it and return it, othwise return original url
	$out = ( strpos( wp_remote_retrieve_body($response) , 'tinyurl.com/' ) ) ? wp_remote_retrieve_body($response) : false; 
	if ( $out ) set_transient( 'pagelines_tinyurl_' . md5( $url ), $out, apply_filters( 'pagelines_tinyurl_cachetimeout', $timeout ) );
	return ( $out ) ? $out : $url;
}
